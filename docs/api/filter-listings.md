...
```bash
curl --request GET \
  --url https://api.visor.vin/v1/listings \
...
| `exclude_inventory_type` | string | Comma-separated inventory classes to exclude: `new`, `used`, `certified`. `cpo` is accepted as an alias for `certified`. |
