...
async fn paginate_dealer_inventory_hits_correct_path() {
    ...
    let items: Vec<_> =
        visor::paginate_dealer_inventory(&client, dealer_uuid(), ListingsFilter::default(), None)
            .collect()
            .await;
...
async fn paginate_dealer_inventory_preserves_non_offset_filter_fields_across_pages() {
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_json(listings_page(&["a"], 0, Some(1))))
        .expect(1)
        .mount(&server)
        .await;
    Mock::given(method("GET"))
